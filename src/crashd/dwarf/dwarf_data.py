...
class AddressMap:
    ...
    def get_file_lines(self):
        return {str(entry.file)+":"+str(entry.line_num)for entry in self._address_low_high}
        ...
        for addr in trace:
            ...
            try:
                if file in file_lines and addr not in smeared:
                    dwarf_data._addr2source[addr] = file_lines[file][line_num].strip()+" => "+file+':'+str(line_num+1)
                    current_source = file_lines[file][line_num]
                    current_file = file
                    current_line = line_num
                elif current_source is not None:
                    dwarf_data._addr2source[addr] = "within " + current_source.strip()+" => "+current_file+':'+str(current_line+1)
                    self._add_cache(addr, current_file, current_line + 1)
                    smeared[addr] = current_source
            except:
                print("err")
                continue
        # pprint(dwarf_data._addr2source)  
    ...
    def __set_address_map(self):
        ...
        for CU in dwarfinfo.iter_CUs():
            ...
            for entry in lineprog.get_entries():
                ...
                prevstate = entry.state   
        address_entries.sort(key=lambda x: x.low)
        address_map._address_low_high = address_entries
        
        for entry in address_entries:
            low, high, file, line = entry.low, entry.high, entry.file, entry.line_num
            address_map._add_cache(low, file, line)
            address_map._source2addr_range[f"{file}{line}"] = (low, high)
            for addr in range(low, high):
                address_map._add_cache(addr, file, line)
        self._address_map = address_map
    ...
    def __setup_file_lines(self):
        for filename in self._address_map.files():
            ...
            if resolved_filename is None:
                continue
            f = open(resolved_filename, "r",encoding="utf8", errors='ignore')
