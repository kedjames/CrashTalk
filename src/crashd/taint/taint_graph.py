...
from crashd.taint.taint_cfg_node import TaintCFGNode
...
class TaintGraph:
    ...
    def get_func_address(self,func_name:str)-> int:
        return [addr for addr, f in self._addr2func.items() if f == func_name]       
    ...
    def _add_uses(
        ...
        # returns a dic with the use str(mem/reg) and the index at which
        #it was defined and the iteration
        #val = None
        ...
        for use, define_idxs in reg_uses.items():
            ...
            for define_idx in define_idxs:
                ...
                if (
                    ...
                    continue
                    inst_addr = self._dataflow.trace._trace[define_idx]
                    self._val_locs[inst_addr] = val
    ...
    def create_taint_cfg_node(self, key: int):
        if len(self._reduced_path) > 0:
            address = key
        else:
            address = self._dataflow.trace._trace[key]
        assembly = self._assembly[address]
        region = self._dataflow.zelos.memory.get_region(address)
        source_code = self.get_source(address)
